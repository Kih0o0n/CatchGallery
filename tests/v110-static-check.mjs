...
assert.match(app, /imageReady:\s*true/);
assert.match(app.match(/async function publishDrawing\(\)[\s\S]*?function expireOldDrawings/)[0], /ref\.set\(data\)[\s\S]*drawingImages[\s\S]*drawingThumbnails/, "complete imageReady metadata must exist before external image writes");
