...
// used for every viewport; no duplicate mobile/desktop canvas or tool DOM exists.
{
  ...
  const values = [state, () => {}, value => String(value), Array.from({ length: 8 }, (_, i) => [`#${i}`, `색${i}`]), appEl, () => {}, document, () => {}, eraser, undo, clearCanvas, nextWord, customWordButton, customWordForm, customCategory, customWord, customAnswers, answerHelpButton, answerHelp, saveDrawing, () => {}, () => {}, () => true, () => true, value => String(value).length, value => String(value), () => {}, () => {}];
