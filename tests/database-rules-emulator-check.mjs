...
const baseDrawing = (overrides = {}) => ({
  ...
  imageReady: true, imageVersion: 1, imageFormat: "webp", imageWidth: 720, imageHeight: 720,
...
const seed = async entries => environment.withSecurityRulesDisabled(async context => {
  await set(ref(context.database()), {
    users: { owner: { nickname: "그린이" }, solver: { nickname: "맞힌이" }, admin: { nickname: "관리자" } },
    admins: { admin: true }, drawings: entries
  });
});
...
try {
  ...
  await assertSucceeds(update(ref(ownerDb), { "userDrawings/owner/new-ok": true }));
  ...
  await assertFails(set(drawingRef(ownerDb, "false-ready"), baseDrawing({ imageReady: false })));
